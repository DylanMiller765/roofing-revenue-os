    ...
    <main className="wrap" style={{paddingTop:32}}>
      ...
      <section className="section">
        <div className="card">
          ...
          <table className="table"><thead><tr><th>Campaign</th><th>Spend</th><th>Qualified</th><th>CPQL</th><th>Booked</th><th>Won</th><th>Status</th></tr></thead><tbody>{rows.map((r) => <tr key={r[0]}>{r.slice(0,6).map((v)=><td key={v}>{v}</td>)}<td><span className={`pill ${r[6]==="Watch"?"warn":r[6]==="Waste"?"bad":""}`}>{r[6]}</span></td></tr>)}</tbody></table>
        </div>
      </section>
